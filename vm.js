  ...
  while (frame.ip !== frame.code.byteLength) {
    ...
    switch (op) {
      ...
      case 0xb9: // invokeinterface // TODO: Optimize if call isn't virtual or interface
        ...
        code += "\
        var Alt = ACCESS_FLAGS.isNative(toCallMethodInfo.access_flags)\n\
                    ? Native\n\
                    : Override.hasMethod(toCallMethodInfo)\n\
                        ? Override\n\
                        : null;\n\
        if (Alt) {\n\
          try {\n\
            Instrument.callPauseHooks(ctx.current());\n\
            Instrument.measure(Alt, ctx, toCallMethodInfo);\n\
            Instrument.callResumeHooks(ctx.current());\n\
          } catch (e) {\n\
            Instrument.callResumeHooks(ctx.current());\n\
            throw e;\n\
          ...
          if (ACCESS_FLAGS.isSynchronized(toCallMethodInfo.access_flags)) {\n\
            callee.lockObject = ACCESS_FLAGS.isStatic(toCallMethodInfo.access_flags)\n\
