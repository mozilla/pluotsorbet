...
bld/native.js: Makefile vm/native/native.cpp vm/native/Boehm.js/.libs/$(BOEHM_LIB) jit/relooper/Relooper.cpp jit/relooper/Relooper.h
	mkdir -p bld
	rm -f bld/native.js
	emcc -DNDEBUG -Ivm/native/Boehm.js/include/ vm/native/Boehm.js/.libs/$(BOEHM_LIB) -Oz -O3 vm/native/native.cpp jit/relooper/Relooper.cpp -o native.raw.js --memory-init-file 0 -s TOTAL_STACK=4*1024*1024 -s TOTAL_MEMORY=$(ASMJS_TOTAL_MEMORY) \
