...
public abstract class AbstractPIMList implements PIMList {
    ...
    public Enumeration itemsByCategory(String category) throws PIMException {
        checkReadPermission(
            Resource.getString(ResourceConstants.JSR75_ENUM_CATEGORY_ITEMS));
        checkOpen();
        Vector v = new Vector();
    ...
    public Enumeration items(PIMItem matchingItem) throws PIMException {
        checkReadPermission(
            Resource.getString(ResourceConstants.JSR75_ENUM_MATCHING_ITEMS));
        checkOpen();
    ...
    throws PIMException {
        checkWritePermission(
            Resource.getString(ResourceConstants.JSR75_DELETE_CATEGORY));
        checkOpen();
        checkNullCategory(category);
    ...
    public Enumeration items() throws PIMException {
        checkReadPermission(
            Resource.getString(ResourceConstants.JSR75_ENUM_ALL_ITEMS));
        checkOpen();
    ...
    throws PIMException {
        checkWritePermission(
            Resource.getString(ResourceConstants.JSR75_RENAME_CATEGORY));
        checkOpen();
    ...
    public Enumeration items(String matchingValue) throws PIMException {
        checkReadPermission(Resource.getString(
            ResourceConstants.JSR75_ENUM_ITEMS_MATCHING_STRING));
        checkOpen();
     ...
     * @throws SecurityException if operation is not permitted
     */
    protected abstract void checkReadPermission(String action)
     ...
     * @throws SecurityException if operation is not permitted
     */
    protected abstract void checkWritePermission(String action)
        throws SecurityException;

    /**
     * Verifies that the given permission is allowed.
     *
     * @param p PIM permission to check
     * @throws SecurityException if the permission is not allowed
     */
    protected void checkPermission(PIMPermission p, String action)
        throws SecurityException {
        AppPackage appPackage = AppPackage.getInstance();
        try {
            appPackage.checkForPermission(new PIMPermission(p.getName(),
                p.getResource() + " '" + name + "' (" + action + ")."));
