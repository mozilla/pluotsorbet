...
public abstract class AbstractPIMItem implements PIMItem {
    ...
    public void addToCategory(String category) throws PIMException {
        ...
        if (categories == null) {
            ...
            if (maxCategories != -1 && maxCategories <= categories.length) {
                throw new PIMException("max categories that this item can " +
                                       "be assigned to is exceeded");
            }
            String[] a = new String[categories.length + 1];
    ...
    public void commit() throws PIMException {
        ...
        pimList.checkWritePermission(
            Resource.getString(ResourceConstants.JSR75_ADD_OR_UPDATE_ITEM));
        pimList.checkOpen();
        updateRevision();
        setDefaultValues();
        try {
    ...
    void remove() throws PIMException {
        ...
        pimList.checkWritePermission(
            Resource.getString(ResourceConstants.JSR75_REMOVE_ITEM));
    ...
    static RuntimeException complaintAboutField(int type, int field) {
        if (isValidPIMField(type, field)) {
            return new UnsupportedFieldException("unsupported field value = " +
                                                 field, field);
        } else {
    ...
    /**
     * If revision is supported, update the revision time.
     */
    private void updateRevision() {
        ...
        try {
            ...
            // revision is not supported, nothing to do
        }
    }

    /**
     * If UID field is supported, update the UID.
     */
    private void updateUID() {
        ...
        try {
            ...
            // UID is not supported, nothing to do
        }
    }
