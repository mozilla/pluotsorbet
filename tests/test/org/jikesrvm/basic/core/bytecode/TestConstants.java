...
class TestConstants implements Testlet {
  public int getExpectedPass() { return 29; }
  ...
  private void aconst() {
    Object x = null;
    th.check(x == null); // aconst_null
  }

  private void iconst() {
    int x;
    x = -1;
    th.check(x, -1); // iconst_m1
    x = 0;
    th.check(x, 0); // iconst_0
    x = 1;
    th.check(x, 1); // iconst_1
    x = 2;
    th.check(x, 2); // iconst_2
    x = 3;
    th.check(x, 3); // iconst_3
    x = 4;
    th.check(x, 4); // iconst_4
    x = 5;
    th.check(x, 5); // iconst_5
  }

  private void lconst() {
    long x;
    x = 0;
    th.check(x, 0); // lconst_0
    x = 1;
    th.check(x, 1); // lconst_1
  }

  private void fconst() {
    float x;
    x = 0;
    th.check(x, 0.0); // fconst_0
    x = 1;
    th.check(x, 1.0); // fconst_1
    x = 2;
    th.check(x, 2.0); // fconst_2
  }

  void dconst() {
    double x;
    x = 0;
    th.check(x, 0.0); // dconst_0
    x = 1;
    th.check(x, 1.0); // dconst_1
  }

  private void misc() {
    byte x0 = 127;
    th.check(x0, 127); // bipush
    x0 = -127;
    th.check(x0, -127); // bipush

    short x1 = 32767;
    th.check(x1, 32767); // sipush
    x1 = -32767;
    th.check(x1, -32767); // sipush

    int x2 = 0x7fffffff;
    th.check(x2, 2147483647); // ldc
    x2 = 0x80000001;
    th.check(x2, -2147483647); // ldc

    long x4 = 0x7fffffffffffffffL;
    th.check(x4, 9223372036854775807L); // ldc2_w
    x4 = 0x8000000000000001L;
    th.check(x4, -9223372036854775807L); // ldc2_w

    th.check(new TestConstants().s(), "X98"); // ldc_w

    // Intel FPU constants
    th.check(Math.PI, 3.141592653589793);

    final double LG2 = 0.3010299956639811952256464283594894482;
    th.check(LG2, 0.3010299956639812);

    final double LN2 = 0.6931471805599453094286904741849753009;
    th.check(LN2, 0.6931471805599453);

    final double L2E = 1.4426950408889634073876517827983434472;
    th.check(L2E, 1.4426950408889634);

    final double L2T = 3.3219280948873623478083405569094566090;
    th.check(L2T, 3.321928094887362);
  }
